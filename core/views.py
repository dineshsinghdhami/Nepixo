...
def friend_suggestions(request):
    ...
    sidebar_suggestions = list(suggestions[:5])
    context = {
        'suggestions': page_obj,
        'sidebar_suggestions': sidebar_suggestions,
...
def send_message(request):
    ...
    if receiver == request.user:
        return JsonResponse({'success': False, 'error': 'You cannot message yourself'}, status=400)
...
def notifications_view(request):
    ...
    sidebar_suggestions = User.objects.exclude(
